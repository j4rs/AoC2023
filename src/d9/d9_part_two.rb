...
def add_placeholders(input, index)
  return input if index == input.length

  if index.zero?
    input[index].prepend(input[index].first)
  else
    input[index].prepend(input[index].first - input[index - 1].first)
  end

  add_placeholders(input, index + 1) # use recursion till reaching last array
end
...
def day9_part_one(input)
  input.map do |i|
    ...
    add_placeholders(extra.reverse, 0) # add placeholders
  end
  .map(&:last) # histories
  .map(&:first) # placeholders
  .sum
end
